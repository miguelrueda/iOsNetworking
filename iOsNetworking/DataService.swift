...
//                let json = try JSONSerialization.jsonObject(with: validData, options: [])
    ...
    func createNewGist(completion: @escaping (Result<Any, Error>) -> Void) {
        ...
        let newGist = Gist(id: nil, isPublic: true, description: "Brand New Gist",
                           files: ["test_file.txt": File(content: "Hello World!")])
        do {
    ...
    func starUnstarGist(id: String, star: Bool, completion: @escaping (Bool) -> Void) {
        let startComponents = createURLComponents(path: "/gists/\(id)/star")
        guard let composedURL = startComponents.url else {
